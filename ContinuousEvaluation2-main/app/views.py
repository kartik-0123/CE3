    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                # If Flask backend is down, delete Django user and show error
                user.delete()
                messages.error(request, "Backend service unavailable. Please try again later.")
        else:
...
def adminn(request):
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            if response.status_code == 200:
                users = response.json()
                print(f"Fetched {len(users)} users from Flask API")
                    ...
                    if selected_user:
                        try:
                            ...
                            if cart_response.status_code == 200:
                                ...
                                print(f"Fetched {len(selected_user_cart)} cart items for user {selected_user['username']}")
                            else:
                                selected_user_cart = []
                                print(f"No cart items found for user {selected_user['username']}")
                        except requests.exceptions.RequestException as e:
                            selected_user_cart = []
                            print(f"Failed to fetch cart items for user {selected_user['username']}: {str(e)}")
        except requests.exceptions.RequestException as e:
            print("API Error:", e)
